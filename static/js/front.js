...
async function getData() {
	const requestURL =
		'https://publy-sora-jeong.github.io/projects/static/js/modalContent.json'
...
async function getData2() {
	const requestURL =
		'https://publy-sora-jeong.github.io/projects/static/js/modalContentOther.json'
	const request = new Request(requestURL)
	const response = await fetch(request)
...
const ariaModal = function ariaModal(projectData) {
	...
	btns_modal.forEach(function(target) {
		target.addEventListener('click', e => {
			...
			let tabAble = modalIDChar.querySelectorAll(
			...
			projectData[modalContentID].forEach(element => {
				//console.log(element);
				modalIDChar.querySelector('.modal__inner__title').innerHTML =
					element.title
				modalIDChar.querySelector('.modal__inner__desc-txt--client').innerHTML =
					element.client
				modalIDChar.querySelector('.modal__inner__desc-txt--link').innerHTML =
					element.link
				modalIDChar.querySelector('.modal__inner__description').innerHTML =
					element.desc
				modalIDChar.querySelector(
					'.modal__inner__content'
				).innerHTML = `<img src="${element.img}" />`
			})
...
var waiAriaTab = function waiAriaTab() {
	...
	tabLists.forEach(function(tabList) {
		tabList.addEventListener('keydown', function(e) {
			...
			var parent =
				tabList.parentNode.tagName === 'LI'
					? tabList.parentNode.parentNode
					: tabList.parentNode
